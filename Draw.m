...
switch settings.model
    ...
    case 'wmr'
        ...
        plot(data.REF(1),data.REF(2),"bx")
        draw_circle(data.OBST_C,data.OBST_R)
        hold off
        figure(2)
        subplot(4,1,1)
        stairs(time,[controls_MPC(:,1), controls_MPC(:,2)])
        legend("al","ar")
        subplot(4,1,2)
        stairs(time,[state_sim(:,4), state_sim(:,5)])
        legend("vl","vr")
        subplot(4,1,3)
        stairs(time,(state_sim(:,4) + state_sim(:,5))/2)
        legend("v")
        subplot(4,1,4)
        stairs(time,(state_sim(:,5) - state_sim(:,4))/0.6)
        legend("w")
